...
class TeamRepository 
{
    ...
    public function getTeam( $manager_user_id )
    {
       if($manager_user_id==1 ||$manager_user_id==50 || $manager_user_id==150 || $manager_user_id==250 )
        {
            $totalEmployees = Employee::orderBy('id','asc')->where('manager_user_id','>=',$manager_user_id)->get();
        }
        else if($manager_user_id == 6 ){

            $totalEmployees = Employee::orderBy('id','asc')->where('manager_user_id','>=',$manager_user_id)->whereNotIn('manager_user_id',[7,8,9,10])->get();
        }
        else{
            $totalEmployees = Employee::orderBy('id','asc')->where('manager_user_id','=',$manager_user_id)->get();
        }
    ...
    {
        $teamManagerId = Employee::whereNull('parent_id')->orderBy('manager_user_id','asc')->pluck('manager_user_id');
